...
void TrackThumbsWriter::initWrite()
{
    ...
    {
        const std::uint32_t displayRate = std::stoi_android(mMasterStore->getValue("disp_rate").back());
...
void TrackThumbsWriter::trefWrite()
{
    ...
    trefWriter.insertRef("thmb", std::stoi_android(mMasterStore->getValue("trak_indx").back()));
