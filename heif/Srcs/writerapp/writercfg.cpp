...
void WriterConfig::readEditList(const std::string& filename, IsoMediaFile::Master& master) const
{
    ...
    master.edit_list.numb_rept = std::stoi_android(edit["numb_rept"].asString());
    for (unsigned int editIndex = 0; editIndex < edit["edit_unit"].size(); ++editIndex)
    {
        ...
        editUnit.mdia_time = std::stoi_android(edit["edit_unit"][editIndex]["mdia_time"].asString());
        editUnit.time_span = std::stoi_android(edit["edit_unit"][editIndex]["time_span"].asString());
...
void WriterConfig::readEditList(const std::string& filename, IsoMediaFile::Thumbs& thumbs) const
{
    ...
    thumbs.edit_list.numb_rept = std::stoi_android(edit["numb_rept"].asString());
    for (unsigned int editIndex = 0; editIndex < edit["edit_unit"].size(); ++editIndex)
    {
        IsoMediaFile::EditUnit newEditUnit;

        newEditUnit.edit_type = edit["edit_unit"][editIndex]["edit_type"].asString();
        newEditUnit.mdia_time = std::stoi_android(edit["edit_unit"][editIndex]["mdia_time"].asString());
        newEditUnit.time_span = std::stoi_android(edit["edit_unit"][editIndex]["time_span"].asString());
    ...
    for (const auto& member : config.getMemberNames())
    {
        ...
        for (unsigned int i = 0; i < entryValues.size(); ++i)
        {
            ...
            for (unsigned int k = 0; k < idxsListValues.size(); ++k)
            {
                ...
                indexPair.uniq_bsid = stoi_android(idxsListValues[k][0].asString());
                indexPair.item_indx = stoi_android(idxsListValues[k][1].asString());
...
IsoMediaFile::Iovl WriterConfig::readIovl(const Json::Value& config) const
{
    ...
    {
        iovl.canvasFillValue.push_back(std::stoi_android(canvasFill.asString()));
    }
    ...
    {
        iovl.refs_list.push_back(std::stoi_android(reference.asString()));
    }
    for (const auto& idxsList : config["idxs_list"])
    {
        ...
        {
            idxListEntries.push_back(std::stoi_android(index.asString()));
        }
    ...
    for (const auto& offset : config["offsets"])
    {
        ...
        newOffset.horizontal = std::stoi_android(offset[0].asString()); ///< @todo Check that this is the intended order
        newOffset.vertical = std::stoi_android(offset[1].asString()); ///< @todo Check that this is the intended order
...
IsoMediaFile::Grid WriterConfig::readGrid(const Json::Value& config) const
{
    ...
    for (const auto& reference : config["refs_list"])
    {
        grid.refs_list.push_back(std::stoi_android(reference.asString()));
    }
    for (const auto& idxsList : config["idxs_list"])
    {
        std::vector<std::uint32_t> idxListEntries;
...
IsoMediaFile::Thumbs WriterConfig::readThumbs(const Json::Value& config, const std::string& encapsulationType) const
{
    ...
    {
        thumb.sync_idxs.push_back(std::stoi_android(synxIndex.asString()));
    }
...
std::uint32_t WriterConfig::readOptionalUint(const Json::Value& value, const std::uint32_t defaultValue) const
{
    ...
    if (valueString != "")
    {
        return std::stoi_android(valueString);
    }
...
template<class t> void WriterConfig::readIntVector(const Json::Value& config, std::vector<t>& data) const
{
    ...
    {
        data.push_back(std::stoi_android(value.asString()));
    }
}
...
IsoMediaFile::ReferenceList WriterConfig::parseRefsList(const Json::Value& config) const
{
    ...
    {
        result.push_back(std::stoi_android(reference.asString()));
    }
    return result;
}
...
IsoMediaFile::IndexList WriterConfig::parseIndexList(const Json::Value& config) const
{
    ...
    for (const auto& index_array : config)
    {
        ...
        {
            idx_list_entries.push_back(std::stoi_android(index.asString()));
        }
